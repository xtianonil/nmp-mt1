<button class="export_as_csv_button server_view tat btn btn-default btn-sm">Export as CSV</button>
				...
				<div class="filter_label">
					<table>
						...
						<tr>
							...
							<td class="filter-group2" class="filter_options">
									...
									<?php
										...
										if (is_array($tcserver_list))
											foreach ($tcserver_list as $column_header)
												echo '<td colspan="2" class="table_custom_header">' . $column_header->tcserver_name . '</td>';
									?>
								</tr>
								<tr>
									<?php
									...
									foreach ($time as $time_interval)
									{
										echo '<tr>';
											...
											if (is_array($tcserver_list))
											{
												...
												{
													echo '<td align="right">';
													if (is_array($rows[$time_interval][$tcserver->tcserver_name]))
													{
														foreach ($rows[$time_interval][$tcserver->tcserver_name] as $row)
														{
															if ($row->time_interval === $time_interval && $row->tcserver_name === $tcserver->tcserver_name)
															{
